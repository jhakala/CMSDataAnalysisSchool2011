...
void plot_sign_step6(){
  ...
  hist_bh2454(var, "Multiplicity>=3", nbins, binmin, binmax, "m3p");
  hsignal[0]=(TH1F*)m3p->Clone();

  hist_bh2454(var, "Multiplicity>=4", nbins, binmin, binmax, "m4p");
  hsignal[1]=(TH1F*)m4p->Clone();

  hist_bh2454(var, "Multiplicity>=5", nbins, binmin, binmax, "m5p");
  hsignal[2]=(TH1F*)m5p->Clone();

  hist_bh2454(var, "Multiplicity>=6", nbins, binmin, binmax, "m6p");
  hsignal[3]=(TH1F*)m6p->Clone();

/*
  TH1F *mp[10];
  //TH1F *m3p,m4p,m5p,m6p;
  //TH1F *mp[0] = new TH1F("m3p","",nbins, binmin, binmax);
  //TH1F *mp[1] = new TH1F("m4p","",nbins, binmin, binmax);  
  //TH1F *mp[2] = new TH1F("m5p","",nbins, binmin, binmax);  
  //TH1F *mp[3] = new TH1F("m6p","",nbins, binmin, binmax);  
  
  for (int i = 3; i < 7; i++) {
    hist_bh234(var, Form("Multiplicity>=%i",i), nbins, binmin, binmax, Form("m%ip",i) );
    TH1F* mp[i] = (TH1F*)m3p->Clone();
    //if (i>3)     TH1F* mp[i-2] = (TH1F*)m3p->Clone();
    //cout<<"histo "<<Form("m%ip",i)<<" "<<m3p->Integral()<<endl;
  }
  cout<<"Test "<<mp[0]->Integral()<<endl;
  hsignal[0] = (TH1F*)mp[3]->Clone();
  hsignal[1] = (TH1F*)mp[4]->Clone();
  hsignal[2] = (TH1F*)mp[5]->Clone();
  hsignal[3] = (TH1F*)mp[6]->Clone();
*/
...
//  draw(hbkg[0],hbkg[1],hbkg[2],hbkg[3],hsignal[0],hsignal[1],hsignal[2],hsignal[3],signif0,signif1,signif2,signif3);
//}

//void draw(TH1F *hbkg[0],TH1F *hbkg[1],TH1F *hbkg[2],TH1F *hbkg[3],TH1F *hsignal[0],TH1F *hsignal[1],TH1F *hsignal[2],TH1F *hsignal[3],TGraph *signif0,TGraph *signif1,TGraph *signif2,TGraph *signif3) {          
