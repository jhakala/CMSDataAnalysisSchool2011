...
void norm_step4(){
  ...
  const TString req 	= "Multiplicity>=5";
  const int nbins 	= 10;
  ...
  if (chisq/ndf > 5) { 
    cout<<"\n!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! "<<endl;
    cout<<"WARNING! BAD FIT! Please, type in the scale into file named "<<Form("Results/normalization_coefficient_%s.txt manually!",namereq)<<endl;  
  ...
  cout<<"Cross check: the scale w/o LLH fitting is "<<scale_nofit<<endl;
